...
struct Tree:
    ...
    fn get_current_node(
        mut self, current_idx: Int, token_bundle: TokenBundle
    ) raises -> Int:
        if len(self.nodes) == 0:
        ...
        if node.done(token_bundle, self):
            ...
            self.nodes[
                current_idx
            ] = node  # NOTE Dumb, cant work with ptr though
            ...
            raise Error(
                "Warning invalid path for token: "
                + String(token_bundle)
                + " current node: "
                + String(node)
                + "\n"
    ...
    for line in path.read_text().split("\n"):
        ...
        for token in (
            ...
            while root_node.reprocess_token_buffer:
                ...
                current_idx = root_node.get_current_node(
                    current_idx, _reprocess_token
                )
