...
from c_binder_mojo.c_ast_nodes_old.nodes import DeletedNode, AstNode
...
struct TypedefNode(NodeAstLike):
    ...
    fn type_name(self) -> TokenBundle:
        return self._token_bundles[
            -2
        ]  # Second to last token, being a semi colon
            ...
            for child in self._children[]:
                ...
                if len(node.token_bundles()) == 1:
                    for token in node.token_bundles():
                        if token[] == name_token:
                            tree.nodes[child[]] = AstNode(
                                DeletedNode(node.token_bundles(), self._parent)
                            )
