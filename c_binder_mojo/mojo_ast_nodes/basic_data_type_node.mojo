...
struct BasicDataTypeNode(NodeAstLike):
    ...
    fn __init__(out self, c_ast_node: c_ast_nodes.nodes.AstNode):
        ...
        self._is_compound = c_ast_node.node[c_ast_nodes.nodes.BasicDataTypeNode].is_compound
    ...
    fn accept(c_ast_node: c_ast_nodes.nodes.AstNode, parent_idx: Int, tree_interface: TreeInterface) -> Bool:
        return c_ast_node.node.isa[c_ast_nodes.nodes.BasicDataTypeNode]()
    ...
    fn display_name(self) -> String:
        return self.__name__ + "(type=" + self._token_bundles[0].token + ", is_compound=" + String(self._is_compound) + ", " + String(self._indices[]) + ")"
    ...
    fn c_type_to_mojo_type(self, c_type: String) -> String:
        if c_type == "int":
            ...
            return "# Complex typedef not supported yet: " + String(self._token_bundles)

    fn finalize(mut self, parent_idx: Int, mut tree_interface: TreeInterface):
        if self._is_compound:
            print("Compound type not supported yet: " + String(self._token_bundles))
            return
