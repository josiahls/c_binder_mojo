...
struct NodeIndices:
    ...
    var mojo_children_idxs: ArcPointer[
        List[Int]
    ]  # Children indices in Mojo AST
    ...
    fn __str__(self) -> String:
        ...
        s += (
            "c_node_idx="
            + String(self.c_node_idx)
            + ", c_parent_idx="
            + String(self.c_parent_idx)
            + ", mojo_node_idx="
            + String(self.mojo_node_idx)
            + ", mojo_parent_idx="
            + String(self.mojo_parent_idx)
        )

        if len(self.c_children_idxs[]) > 0:
            s += ", c_children_idxs=" + self._child_str(self.c_children_idxs)
        if len(self.mojo_children_idxs[]) > 0:
            s += ", mojo_children_idxs=" + self._child_str(
                self.mojo_children_idxs
            )
        return s
    ...
    fn __init__(
        ...
        mojo_node_idx: Int,
        mojo_parent_idx: Int,
        c_children_idxs: ArcPointer[List[Int]],
    ):
        self.c_node_idx = c_node_idx
        self.c_parent_idx = c_parent_idx
