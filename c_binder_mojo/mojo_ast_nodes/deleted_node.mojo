...
struct DeletedNode(NodeAstLike):
    ...
    fn __str__(self) -> String:
        ...
        if self._as_comment:
            s += node2string(
                self.display_name(), self._token_bundles, self._str_just_code
            )
            s += "\n# " + self._reason
        else:
            s += self._reason
...
fn recursive_delete(
    mut tree_interface: TreeInterface,
    node_idx: Int,
    reason: String,
    as_comment: Bool,
):
    ...
    tree_interface.nodes[][node_idx] = AstNode(
        DeletedNode(node, reason, as_comment)
    )
