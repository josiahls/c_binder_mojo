...
struct EnumNode(NodeAstLike):
    ...
    fn __init__(out self, c_ast_node: c_ast_nodes_old.nodes.AstNode):
        ...
        self._enum_name = c_ast_node.node[
            c_ast_nodes_old.nodes.EnumNode
        ].enum_name
    ...
    fn accept(
        ...
        return c_ast_node.node.isa[c_ast_nodes_old.nodes.EnumNode]()
    ...
    fn finalize(mut self, parent_idx: Int, mut tree_interface: TreeInterface):
        ...
        # Add struct header
        mojo_format.append(
            TokenBundle(token="# Mojo Enum", line_num=line_num, col_num=col_num)
        )
        line_num += 1
        mojo_format.append(
            TokenBundle(
                token="struct " + self._enum_name + ":",
                line_num=line_num,
                col_num=col_num,
            )
        )
        ...
        mojo_format.append(
            TokenBundle(
                token="", line_num=line_num, col_num=col_num, is_splitter=True
            )
        )
