...
struct StructFieldNode(NodeAstLike):
    ...
    fn __init__(out self, c_ast_node: c_ast_nodes_old.nodes.AstNode):
        ...
        self._field_name = c_ast_node.node[
            c_ast_nodes_old.nodes.StructFieldNode
        ].field_name
        self._field_type = ""  # Will be set in finalize
        self._parent_struct_name = ""
        self._is_struct = c_ast_node.node[
            c_ast_nodes_old.nodes.StructFieldNode
        ]._is_struct
    ...
    fn accept(
        ...
        return c_ast_node.node.isa[c_ast_nodes_old.nodes.StructFieldNode]()
    ...
    fn is_complete(
        ...
        if self._is_struct:
            return (
                c_ast_node.current_idx()
                not in self._indices[].c_children_idxs[]
            )
        return True
        ...
        if self._is_struct:
            ...
            while _parent_idx >= 0:
                ...
                if parent_node.node[].isa[StructNode]():
                    self._parent_struct_name = parent_node.node[][
                        StructNode
                    ]._struct_name
                    break
            ...
            if len(self._indices[].mojo_children_idxs[]) > 0:
                for struct_child_idx in self._indices[].mojo_children_idxs[]:
                    if (
                        tree_interface.nodes[][struct_child_idx[]]
                        .node[]
                        .isa[StructNode]()
                    ):
                        var struct_node = tree_interface.nodes[][
                            struct_child_idx[]
                        ]
                        # Refresh the struct node's formatting before moving parents
                        struct_node.node[][
                            StructNode
                        ]._struct_name = self._field_type
                        ...
                        for i in range(
                            len(self._indices[].mojo_children_idxs[])
                        ):
                            if (
                                self._indices[].mojo_children_idxs[][i]
                                == struct_child_idx[]
                            ):
