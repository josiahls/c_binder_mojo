...
struct Tree:
    ...
    fn get_current_node(
        mut self, current_idx: Int, c_ast_node: c_ast_nodes_old.nodes.AstNode
    ) raises -> Int:
        # print('Processing node: ' + String(c_ast_node.display_name()) + ' current_idx: ' + String(current_idx))
        tree_interface = TreeInterface(
            self.nodes, self.c_macro_defs, self.mojo_aliases
        )
        # Initialize if empty
        if len(self.nodes[]) == 0:
            # print("Creating root node")
            self.nodes[].append(
                AstNode(RootNode.create(c_ast_node, -1, tree_interface))
            )
            return 0
        ...
        if current_node.is_accepting_tokens(c_ast_node, tree_interface):
            ...
            current_node.finalize(
                current_node.indices()[].mojo_parent_idx, tree_interface
            )
            return self.get_current_node(
                current_node.indices()[].mojo_parent_idx, c_ast_node
            )

        elif current_node.wants_child(c_ast_node, tree_interface):
            new_node = AstNode.accept(c_ast_node, current_idx, tree_interface)
            new_node.indices()[].mojo_parent_idx = (
                current_node.indices()[].mojo_node_idx
            )
...
fn make_tree(
    input_tree: c_ast_nodes_old.tree.Tree, c_macro_defs: List[String]
) raises -> Tree:
