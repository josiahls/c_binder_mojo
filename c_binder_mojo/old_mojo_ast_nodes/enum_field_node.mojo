...
struct EnumFieldNode(NodeAstLike):
    ...
    fn __init__(
        ...
        parent_enum_values: ArcPointer[List[UInt]],
    ):
        ...
        self._field_name = (
            c_ast_node.node[c_ast_nodes_old.nodes.EnumFieldNode]
            .token_bundles()[0]
            .token
        )
        ...
        if (
            len(
                c_ast_node.node[
                    c_ast_nodes_old.nodes.EnumFieldNode
                ].token_bundles()
            )
            == 1
        ):
            ...
            var field_value = c_ast_node.node[
                c_ast_nodes_old.nodes.EnumFieldNode
            ].field_value
            ...
            print(
                "EnumFieldNode: Unexpected number of token bundles: "
                + String(
                    len(
                        c_ast_node.node[
                            c_ast_nodes_old.nodes.EnumFieldNode
    ...
    fn create(
        ...
        if node[].isa[ScopeNode]():
            node = tree_interface.nodes[][
                node[][ScopeNode]._indices[].mojo_parent_idx
            ].node
