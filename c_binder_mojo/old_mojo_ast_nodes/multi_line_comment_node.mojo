...
struct MultiLineCommentNode(NodeAstLike):
    ...
    fn accept(
        ...
        return c_ast_node.node.isa[c_ast_nodes_old.nodes.MultiLineCommentNode]()
        ...
        for token in self._token_bundles:
            ...
            if token[].token == "\n":
                ...
                mojo_format_token_bundles.append(
                    TokenBundle(
                        token=token[].token, line_num=line_num, col_num=col_num
                    )
                )
                continue
            ...
            if col_num == 0:
                mojo_format_token_bundles.append(
                    TokenBundle(token="#", line_num=line_num, col_num=col_num)
                )
                col_num += 1
                mojo_format_token_bundles.append(
                    TokenBundle(token=" ", line_num=line_num, col_num=col_num)
