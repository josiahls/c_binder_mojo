...
struct TypedefNode(NodeAstLike):
    ...
    fn __init__(
        ...
        self._alias_name = (
            c_ast_node.node[c_ast_nodes_old.nodes.TypedefNode].type_name().token
        )
    ...
    fn accept(
        ...
        return c_ast_node.node.isa[c_ast_nodes_old.nodes.TypedefNode]()
    ...
    fn finalize(mut self, parent_idx: Int, mut tree_interface: TreeInterface):
        ...
        if len(self._indices[].mojo_children_idxs[]) == 0:
            print(
                "Warning: Typedef has no children: "
        ...
        if child_node.node[].isa[BasicDataTypeNode]():
            ...
            for token in List[String](
                ...
                self._alias_name,
                "=",
                child_node.token_bundles()[0].token,
            ):
                mojo_format_token_bundles.append(
                    TokenBundle(
                        token=token[], line_num=line_num, col_num=col_num
                    )
                )
                col_num += len(token[]) + 1

            # Delete the child from the tree since it's been handled
            tree_interface.nodes[][child_idx] = AstNode(
                DeletedNode(
                    ast_node=child_node,
                    reason=(
                        "Deleted because it has been consolidated into a mojo"
                        " alias"
                    ),
                    as_comment=True,
                )
            ...
            for token in List[String](
                "alias", self._alias_name, "=", enum_name
            ):
                mojo_format_token_bundles.append(
                    TokenBundle(
                        token=token[], line_num=line_num, col_num=col_num
            ...
            for token in List[String](
                "alias", self._alias_name, "=", struct_name
            ):
                mojo_format_token_bundles.append(
                    TokenBundle(
                        token=token[], line_num=line_num, col_num=col_num
            ...
            mojo_format_token_bundles.append(
                TokenBundle(
                    token="# Complex typedef not supported yet: "
                    + String(self._token_bundles),
                    line_num=line_num,
                    col_num=0,
                )
