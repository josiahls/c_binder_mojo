...
struct anonymous_declaration_node(Copyable, Movable):
    ...
    fn __init__(out self):
        try:
            self.lib = DLHandle(_get_lib_path('libanonymous_declaration_node.so'))
        except e:
            self.lib = abort[DLHandle](
                String("Failed to load anonymous_declaration_node from", 'libanonymous_declaration_node.so', ":\n", e)
            )
