...
@fieldwise_init
struct enum_decl_node(Copyable, Movable):
    var lib: DLHandle
    
    var use_enum_value: enum_decl_node_use_enum_value.type

    fn __init__(out self):
        try:
            self.lib = DLHandle(_get_lib_path('libenum_decl_node.so'))
        except e:
            self.lib = abort[DLHandle](
                String("Failed to load enum_decl_node from", 'libenum_decl_node.so', ":\n", e)
            )

    
        self.use_enum_value = enum_decl_node_use_enum_value.load(self.lib)
