...
fn test_ifndef_node() raises:
    ...
    var test_dir = Path("/home/fastrl_mojo_user/fastrl_mojo/c_binder_mojo/tests/test_ifndef_node")
        ...
        if node_name == "MacroIfNDefNode":
            ...
            for j in range(len(indices.original_child_idxs)):
                ...
                if child_node.name() == "MacroIfNDefNode":
                    ...
                    logger.info("Found nested ifndef node: " + child_node.name(include_sig=True))
    ...
    if ifndef_count < 8:
        raise Error("Expected at least 8 ifndef nodes, but found " + String(ifndef_count))
    
    # We should have at least 1 nested ifndef
    if nested_ifndef_count < 1:
        raise Error("Expected at least 1 nested ifndef, but found " + String(nested_ifndef_count))
    
    # We should have at least 8 define nodes (one for each ifndef)
    if define_count < 8:
        raise Error("Expected at least 8 define nodes, but found " + String(define_count))
