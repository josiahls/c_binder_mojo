...
fn test_multiline_comment_node() raises:
    ...
    for i in range(len(tokenizer.tokens)):
        ...
        token_debug_content += "Token " + String(i) + ": '" + token.token + "'\n"
    ...
    for i in range(len(tree_interface.nodes()[])):
        ...
        if tree_interface.nodes()[][i].name() == "MultilineCommentNode":
            ...
            if "/**" in node_str:
                doc_comment_count += 1
                logger.info("Found documentation comment: " + node_str[:40] + "...")
            else:
                logger.info("Found regular multiline comment: " + node_str[:40] + "...")
    
    # We expect at least 10 multiline comments in our test file (not counting the single line comment)
    if comment_count < 10:
        raise Error("Expected at least 10 multiline comments, but found " + String(comment_count))
