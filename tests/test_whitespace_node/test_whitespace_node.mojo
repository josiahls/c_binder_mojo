...
fn test_whitespace_node() raises:
    ...
    var test_dir = Path("/home/fastrl_mojo_user/fastrl_mojo/c_binder_mojo/tests/test_whitespace_node")
    ...
    for i in range(len(tokenizer.tokens)):
        ...
        token_debug_content += "'" + visual + "' (length: " + String(len(token.token)) + ")\n"
    ...
    for i in range(len(tree_interface.nodes()[])):
        ...
        if tree_interface.nodes()[][i].name() == "WhitespaceNode":
            whitespace_count += 1
            var node_str = tree_interface.nodes()[][i].to_string(just_code=True, tree_interface=tree_interface)
    ...
    if whitespace_count < 5:
        raise Error("Expected at least 5 whitespace nodes, but found " + String(whitespace_count))
    
    logger.info("Whitespace test passed with " + String(whitespace_count) + " total whitespace nodes:")
