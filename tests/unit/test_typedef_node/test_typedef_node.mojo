...
fn test_typedef_node() raises:
    ...
    var test_dir = Path(
        "/home/c_binder_mojo_user/c_binder_mojo/tests/unit/test_typedef_node"
    )
    ...
    # Generate Mojo AST
    var mojo_tree_log_file = output_dir / "test_typedef_node_mojo.tree"
    var mojo_module_interface = make_mojo_tree(module_interface.nodes()[], String(mojo_tree_log_file), validate_tree=False)
    mojo_module_interface.nodes()[][0].node[][RootNode]._add_main_function = True

    # Save Mojo AST for debugging
    var mojo_ast_file_just_code = output_dir / "test_typedef_node.mojo"
    ...
    var mojo_ast_file = output_dir / "test_typedef_node.mojo_ast"
        ...
        if node.name() == "TypedefNode":
            ...
            if len(node.indicies().c_child_idxs) > 0:
                ...
                if child_node.name() == "EnumNode":
                    ...
                    logger.info(
                        "Found struct node name: "
                        + child_node.node[][StructNode]._struct_name
                    )
    ...
    if basic_typedef_count != 6:  # uint_t, int64_t, byte_t, integer_t, float_t
        raise Error(
            "Expected 6 basic typedefs, but found "
            + String(basic_typedef_count)
        )
    ...
    if inline_struct_count != 2:  # Point2D, NestedStruct
        raise Error(
            "Expected 2 inline struct typedefs, but found "
            + String(inline_struct_count)
        )
    if named_struct_count != 1:  # Rectangle
        raise Error(
            "Expected 1 named struct typedef, but found "
            + String(named_struct_count)
        )
    if typedefs_in_struct > 0:
        raise Error(
            "Found "
            + String(typedefs_in_struct)
            + " typedefs incorrectly inside struct nodes"
        )
